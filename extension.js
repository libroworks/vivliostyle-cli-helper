...
function activate(context) {
  let disposable = vscode.commands.registerCommand(
    'vivliostyle-cli-helper.helloWorld',
    function () {
      // The code you place here will be executed every time your command is executed

      // Display a message box to the user
      vscode.window.showInformationMessage(
        'Hello World from vivliostyle-cli-helper!'
      );
      // callShell('ls');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'vivliostyle-cli-helper.installCLI',
      function () {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'vivliostyle-cli-helper.previewByConfig',
      function () {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'vivliostyle-cli-helper.buildByConfig',
      function () {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'vivliostyle-cli-helper.previewThis',
      function () {
        const editor = vscode.window.activeTextEditor;
        if (checkEditorPath(editor) === false) return;
        // プレビューしたいパスやVSmodeを設定
        const contentpath = editor.document.fileName.replace(/^[a-z]:/, (d) =>
          d.toUpperCase()
        );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'vivliostyle-cli-helper.buildThis',
      function () {
        const editor = vscode.window.activeTextEditor;
        if (checkEditorPath(editor) === false) return;
        // ビルドしたいパスやVSmodeを設定
  ...
  function callShell(shellcommand) {
    const term =
      vscode.window.activeTerminal?.name === 'vivliostyle-cli-helper'
        ? vscode.window.activeTerminal
        : vscode.window.createTerminal('vivliostyle-cli-helper');
    term.show();
    // PowerShellかつvivliostyleスクリプトの実行時のみ許可が必要
    if (
      vscode.env.shell.includes('powershell') &&
      shellcommand.indexOf('vivliostyle') === 0
    ) {
